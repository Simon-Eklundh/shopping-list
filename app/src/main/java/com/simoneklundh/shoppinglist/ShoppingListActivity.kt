...
class ShoppingListActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val listIdExtra = intent.getStringExtra(EXTRA_LIST_ID)
        if (listIdExtra == null) {
            finish()
            return
        }
        ...
        items.addAll(repository.loadItems(listId))

        val toolbar = findViewById<MaterialToolbar>(R.id.toolbar)
        toolbar.title = intent.getStringExtra(EXTRA_LIST_NAME) ?: getString(R.string.app_name)
    ...
    private fun addItem() {
        ...
        if (existingIndex != -1) {
            ...
            if (existing.checked) {
                ...
                repository.saveItems(listId, items)
                refreshList()
            } else {
                Snackbar.make(
